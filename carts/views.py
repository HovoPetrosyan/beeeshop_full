        ...
        if request.method == 'POST':
            for item in request.POST:
                ...
                try:
                    ...
                    pass
        # if the product exists in the cart then we increase product qty by 1
        is_cart_item_exists = CartItem.objects.filter(product=product, user=current_user).exists()
        if is_cart_item_exists:
            cart_item = CartItem.objects.filter(product=product, user=current_user)
            # existing_variations -> database
            # cuurent variation -> product_variation
            # item_id - > database

            # if cuurent variation is in the existing variation, then we incremet cart item product quantity
            ...
            # if the cartitem is not exist, then we creating cartitem
